...
def compare_file_list(src_file_list, dst_file_list, compare_fun):
    ...
    for src_file in src_file_list:
        src_content = tft_file.load_file_as_list(src_file)
        
        action = 'no-found'
        
        for dst_file in dst_file_list:
            dst_content = tft_file.load_file_as_list(dst_file)
...
def diff_text(src_context, dst_context):
    ...
    src_context = [src_text.strip() for src_text in src_context]
    dst_context = [dst_text.strip() for dst_text in dst_context]
